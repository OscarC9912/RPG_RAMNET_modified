...
class EventDataset(Dataset):
    ...
    def __init__(self, base_folder, event_folder, start_time=0, stop_time=0, transform=None, normalize=True):
        ...
        self.local_dir_dvs = data["dvs_voxel"]
        self.remote_dir_dvs = self.event_folder
        format = '*_voxel.npy'
        self.filepath_dvs = self.loader.list_dir(self.remote_dir_dvs, format)

        format = '*_events.npy'
        self.filepath_event = self.loader.list_dir(self.remote_dir_dvs, format)
    ...
    def read_timestamps(self):
        ...
        # print("Path for stmp:", self.filepath_stmp)
        raw_stamps = self.loader.timestampLoader(self.filepath_stmp, self.local_dir_stmp)
        # copier.sftp.close()
        # modification done
...
class RawEventsDataset(EventDataset):
    ...
    def __getitem__(self, i, transform_seed=None):
        ...
        # Here I make some modification
        events_raw = self.loader.rgbReader(self.filepath_dvs, self.remote_dir_dvs)
        # copier.sftp.close()
        # modification done
