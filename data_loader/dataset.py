    ...
    def __getitem__(self, i, seed=None):
        ...
        for k in range(0, self.every_x_rgb_frame):
            ...
            if self.use_mvsec:
                ...
                # Here I make some modification
                frame = self.loader.depthFrameLoader(self.filepath_loop, self.local_dir_loop)
                # modification done
                ...
                if k == 0:  # remove this if statement for ergb ideal!
                    if frame_idx < self.every_x_rgb_frame:
                        ...
                        if self.use_mvsec:
                            ...
                            # Here I make some modification
                            rgb_frame = self.loader.rgbReader(self.filepath_rgb, self.local_dir_rgb)
                            # modification done
                ...
                if self.frame_folder is not None:
                    try:
                        if self.use_mvsec:
                            ...
                            # Here I make some modification
                            rgb_frame = self.loader.rgbReader(self.filepath_rgb2, self.local_dir_rgb)
                            # modification done
